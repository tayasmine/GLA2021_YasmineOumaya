...
public class PilotResource implements PilotDAO {
	 ...
	 * 
	 * @Path("/Pilot/getpilotbyid/{name}") public String
	 * getPilotDetails(@PathParam("name") String name) { List<Pilot> pilots =
	 * DataBase.getAllPilots(); if (pilots == null) { throw new NotFoundException();
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/Pilot/{name}")
	public List<Pilot> getElement(@PathParam("name") String name) {
		List<Pilot> pilots = DataBase.getAllPilots();
		if (pilots == null) {
			throw new NotFoundException();
		}
	 ...
	 * @Consumes(MediaType.APPLICATION_JSON)
	 * 
	 * @Path("/Pilot/Edit/{name}") public void editElement(@PathParam("name") String
	 * name, Pilot elt) { DAOFactory.getPilotDAO().editElement(name, elt); }
	 */
}
